...
In addition, CRExplorer is written in Java, so you need to install it as well. Download it from here [Oracle](https://www.oracle.com/java/) or here [OpenJDK](https://openjdk.java.net/).
...
We start with a dataset of references. Here, the data for optogenetis was obtained by snowballing from the paper that established the method (Boyden ES, Zhang F, Bamberg E, Nagel G, Deisseroth K. 2005. "Millisecond-timescale, genetically targeted optical control of neural activity". Nature Neuroscience). These records, retrieved from Web of Science, are available with this package, as an example. The records contain the 2005 paper, the references cited by this paper and the references cited by those references (i.e. second generation citations).
